...
class AvalonApp:
    ...
    def show_actual_role(self):
        ...
        info = ""
        if r == 'Merlin':
            ev = [x for x,y in self.roles.items() if y in ['Evil','Assassin','Morgana']]
            info = "Evil guys:\n" + "\n".join(ev)
        elif r == 'Percival':
            ml = [x for x,y in self.roles.items() if y in ['Merlin','Morgana']]
            random.shuffle(ml)
            info = "Merlin is one of:\n" + "\n".join(ml)
        elif r in ['Evil','Assassin','Morgana']:
            mates = [x for x,y in self.roles.items() if y in ['Evil','Assassin','Morgana'] and x!=p]
    ...
    def start_team_proposal(self):
        ...
        tk.Button(self.root, text="Submit Team", command=lambda: self.ask_vote(ts)).pack(pady=10)
        # persistent summary
        evil = {5:2,6:2,7:3,8:3,9:3,10:4}[n]
        good = n - evil
        summary = (
        ...
        tk.Label(self.root, text=summary, font=("Arial",8), fg='gray').pack(side='bottom', pady=5)
    ...
    def show_next_mission_vote(self):
        ...
        is_evil = self.roles[p] in ['Evil','Assassin','Morgana']
